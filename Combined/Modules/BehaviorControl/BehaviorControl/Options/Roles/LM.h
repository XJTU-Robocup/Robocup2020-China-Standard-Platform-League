option(LM)
{
  ...
  state(alignBehindBall)
  {
    ...
    action
    {
      ...
      WalkToTarget(Pose2f(0.5f, 0.5f, 0.5f), Pose2f(theLibCodeRelease.angleToGoal, theBallModel.estimate.position.x() - 145.f, theBallModel.estimate.position.y()-40.f));
    }
  }
  ...
  state(walkToShootPoint)
  {
    transition
    {
      if(modeDecision22()!=1)
        goto stateDecision;
      if(theLibCodeRelease.between(theBallModel.estimate.position.y(), -200.f, 200.f)
         && theLibCodeRelease.between(theBallModel.estimate.position.x(), -200.f, 200.f))
        goto shoot;
    }
    action
    {
      ...
      WalkToTarget(Pose2f(0.5f, 0.5f, 0.5f), Pose2f(kickTarget.angle(), theBallModel.estimate.position.x() - 165.f, theBallModel.estimate.position.y() - 42.f));
    }
  }
  ...
  state(kick) 
  {
    transition
    {
      if(state_time > 3000 || (state_time > 100 && action_done))
        goto stateDecision;
    }
    action
    {
